...
description: The phone path end to end — the TelegramBotAPI client and its two error kinds (NotifyRejected versus TelegramAPIError), callback data bound to the card version inside Telegram's 64-byte limit, the keyboard attach/strip lifecycle across restarts, owner-only update filtering and the /config stub, and the long-polling loop with a durable offset.
...
version, the message and keyboard lifecycle, owner-only filtering, and what
`/config` actually does today.
...
`TelegramBotAPI` holds one URL, `https://api.telegram.org/bot<token>/`, and four
...
| `send_message(chat_id, text, reply_markup, parse_mode)` | `chat_id`, `text`, `parse_mode` only when not `None`, and `reply_markup` only when not `None` | must be an object (the adapter reads `message_id` from it) |
...
| a `result` of the wrong shape (updates not a list, sent message not an object) | `TelegramAPIError` |
...
- `edit_and_strip(card_id, version)` ignores the version and strips the message
  ...
  `update_request` superseded. `Inbox._request_strip` swallows a failure: the
  answer is already durable and a dead strip is an honest miss, not a failed
  answer.
...
2. for each card: an open card past its deadline is expired (and stripped); an
   ...
   the reservation is on record; a card that is no longer open is stripped.
...
resume exactly where the first stopped without probing again.
...
- `_require_update_id` accepts only a plain non-negative `int`. Strings, floats,
  booleans, `int` subclasses, `IntEnum` members and negatives all raise
  `TelegramAPIError`. A malformed entry makes the whole poll that error, while
  the entries already handled keep their persisted offset — the loop backs off
  and retries from the failed entry rather than skipping it.
...
| a renotified card's buttons carry the bumped version | `tests/inbox/test_telegram_port.py::test_renotify_sends_complete_updated_card_and_persists_message` |
| a stale button after a renotify strips only its own message | `tests/inbox/test_telegram_port.py::test_stale_callback_after_renotify_strips_only_old_message` |
...
| a cancel during the attach strips the buttons | `tests/inbox/test_telegram_port.py::test_cancellation_during_keyboard_attachment_strips_buttons` |
...
| first boot skips the queued backlog and persists the offset | `tests/inbox/test_runtime.py::test_first_boot_bootstraps_without_dispatching_queued_updates`, `::test_bootstrap_persists_next_offset_across_restart` |
...
| coerced `update_id` values are rejected | `tests/inbox/test_runtime.py::test_bootstrap_rejects_coerced_update_ids`, `::test_poll_rejects_coerced_update_ids_without_skipping` |
...
| `ok: false` is `NotifyRejected`; transport failures are `TelegramAPIError` | `tests/inbox/test_runtime.py::TestTelegramBotAPI` |
...
shipped control flow rather than a mock of it; the only thing faked is the
network and the clock. `tests/inbox/test_runtime.py` does the same for the
transport and the loop, patching `TelegramBotAPI` with a scripted stand-in while
using the real `Runtime`, store and adapter. The reply branch, the renderer and
the claim ladder have their own modules
